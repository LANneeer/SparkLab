...
async def send_ride(message: types.Message, state: FSMContext):
    ...
    if not rides:
        await message.answer(
            text="К сожалению, ближайших поездок нет😔", reply_markup=user_menu
        )
        await state.clear()
    else:
        await message.answer(
            ...
            reply_markup=types.ReplyKeyboardMarkup(
                keyboard=[[types.KeyboardButton(text=ride)] for ride in rides],
...
async def get_ride(message: types.Message, state: FSMContext):
    ...
    rides = Ride.objects.filter(
        departure__gte=departure, departure__lte=departure + timezone.timedelta(days=1)
    )
    rides = rides.order_by("departure")
    rides = rides.values("pk", "departure", "ride_title")
    rides = [
        f"{ride['pk']} - {ride['ride_title']} - {ride['departure'].strftime('%H:%M')}"
        for ride in rides
    ]
...
async def confirm_payment(message: types.Message, state: FSMContext):
    ...
    if message.text == "Отменить🛑":
        await message.answer(text="Поездка отменена🛑", reply_markup=user_menu)
        ...
        await message.bot.send_message(
            ...
            f"<b>Количество пассажиров:</b> <i>{ride.ride.user.count()}</i>\n"
                ...
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text="Подтвердить🟢", callback_data=f"confirm_{ride.id}"
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
                            text="Отклонить🛑", callback_data=f"decline_{ride.id}"
                        )
                    ],
                ]
            ),
...
async def my_rides(message: types.Message):
    user = User.objects.get(telegram_id=message.from_user.id)
    rides = []
    for ride in user.rides.filter(
        departure__gte=timezone.now(), ride_requests__status__in=["pending", "approved"]
    ):
        status = RideRequest.objects.filter(ride=ride, user=user).first().status
        text = (
            f"<b>Название поездки:</b> <i>{ride.ride_title}</i> \n"
    ...
    if rides:
        for ride in rides:
            await message.answer(text=ride, reply_markup=user_menu)
    else:
        await message.answer(text="У вас пока нет поездок😔", reply_markup=user_menu)
...
async def send_comment(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        text="Напишите ваш отзыв💬:", reply_markup=types.ReplyKeyboardRemove()
    )
...
async def write_comment(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        text="Ваш отзыв отправлен. Спасибо за обратную связь!❤", reply_markup=user_menu
    )
    await state.clear()
